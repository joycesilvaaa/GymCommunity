...
from sqlalchemy.sql import bindparam, text, update, delete

from app.core.db_model import User, UserRelations
from app.schemas.user import CreateUser, UpdateUser, UserResponse, UserClientsForProfessional, UserDetail
...
class UserService:
    ...
    async def _create_user_relation(self, user_id: int, professional_id: int) -> None:
        user_relation = UserRelations(user_id=user_id, professional_id=professional_id)
    ...
    async def get_user_by_cpf(self, cpf: str) -> UserResponse | None:
        query = text(
            "SELECT id, email, password, user_profile FROM users WHERE cpf = :cpf"
    ...
    async def get_clients_for_professional(self, user_id: int) -> list[UserClientsForProfessional]:
        query = text(
            """select 
                    u.id,
                    u."name" ,
        ...
        return [
            UserClientsForProfessional(**client._asdict())
...
"""
    ...
    async def delete_relation(self, user_id: int, professional_id: int) -> None:
        await self._session.execute(
            delete(UserRelations).where(UserRelations.user_id == user_id and UserRelations.professional_id == professional_id)
        )
        await self._session.commit()